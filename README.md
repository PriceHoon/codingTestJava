...
# 4. 올바른 괄 - Level2(Solved)
# 5. 최소값 만들기 - Level2(Solved)
(처음에 dp문제인줄 알았으나 쉬운개념이였음..)
# 6. 이진변환 반복하기 - Level2(Solved)
(약간 야매풀이인듯)
...
# 12. 완전탐색(카펫) - Level2(Solved)
(약수 접근으 맞았으나, 그 중에서 판별하는 과정의 실수가 있었다.)
# 13. 예상 대진표 - Level2(Solved)
(대진표 문제 - 같은 대진에 들어갈 경우를 구하는 = 4,5번 일 경우를 생각 못해서 +1이 같냐?로 풀다가 예외에 걸림
조금 더 생각하자!)
# 14. 구명보트 - Level2(Solved)
(풀이를 참고하기도 했고, 굳이 지워주는 것이 아닌 index를 -- 해주면서 할 수 있는 방법이 있는데..)
# 15. 점프와 순간이동 - Level2(Solved)
# 16. N개의 최소공배수 - Level2(Solved)
(여러숫자의 최소공배수를 구한다고 했을 때 최대공약수를 먼저 생각했고, 모든 수의 최대공약수를 한번에 구하려고해서 문제가 됐다.
처음부터 차례대로 최소공배수를 구해서 최종에 도달해야한다. -> 이 때 유클리드 호재법을 사용했다.)
# 17. 멀리뛰기 - Level2(Solved)
(n=1일때 조건을 걸어주지 않아서 1개의 오류가 발생했었다. 주의하자)
# 18. 귤 고르기 - Level2(Solved)